...
If tbing=1 Then
 ...
 If googleerror=0 Then 
	...
	'Next
	'If Left(mwtext,4)<>"����" Then guinotice "����"
...
If webidata>2 Then'And tbing=1 Then
	'guinotice Str(Asc("�"))+" "+Str(Asc("�"))+"  "+Str(&hD9)+" "+Str(recvdata(webidata-1))'217
 	     	 	...
 	     	 	Else
 	     	 		...
 	     	 		Else
 	     	 			...
 	     	 			If j<=0 Then 
     	  	     	    ...
     	  	     	    If myiasknode<mynasknode Then'And (trunway<>0) Then
    	  	     		   myiasknode+=1
    	  	     		   myasknodeid(myiasknode)=id
     	  	     	    EndIf 
     	  	     	   EndIf  
...
'EndIf
 Var zz=z,zz0=z
 If testcrops=1 Then zz0=getterrainheight(x,y)+h:kwaynodez(1)=zz0:z=zz0
...
Else
 ...
 y=y0'townwaynodey(ij,i,1)
 z=zz0
 Var zmid=z1:If testcrops=1 Then zmid=(kwaynodez(1)+kwaynodez(Int(n*0.5)))*0.5
 'zmid+=300
 Var tx=0.025
 Var testcull=0,testcull0=0,xcull=x,ycull=y
 glenable(gl_cull_face)
 glcullface(gl_back)
 'If troof=1 Then  
 ...
 For j=2 To min2(n,nwaynode-1)
 	xx=x:yy=y:zz=z
 	x=townwaynodex(ij,i,j)
 	y=townwaynodey(ij,i,j)
 	'gltriangle xx,yy,z, xmid,ymid,z1, x,y,z
 	/'Var dxy=(y-ymid)*(xcull-xmid)-(x-xmid)*(ycull-ymid)
 	testcull=0
 	If dxy>1 Then
 		testcull=1
 	Else 
