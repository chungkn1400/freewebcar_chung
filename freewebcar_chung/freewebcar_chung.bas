...
Dim Shared As double auxvar,auxvar2,auxvar3,auxvar4,auxvar5,auxvar6,auxtest0=0.8,auxtest=0
...
Dim Shared As Integer tshowradio,tradio
...
Sub initsounds
   ...
   soundfic="sounds/voyage_enfer.mp3"
   ...
   soundfic="sounds/heresun.mp3"
   mcisendstring("open "+chr$(34)+soundfic+chr$(34)+" shareable alias heresun",0,0,0)
...
Sub closesounds
   ...
   mcisendstring("close avion2",0,0,0)
	...
	mcisendstring("close heresun",0,0,0)
...
Sub setsoundvol
	...
	mcisendstring("setaudio arcade volume to "+str(Int(2.5*vol)),0,0,0)'2.5'1.4'1.95'1.4
	mcisendstring("setaudio crash volume to "+str(vol),0,0,0)
	...
	mcisendstring("setaudio heresun volume to "+Str(Int(2.4*vol)),0,0,0)
...
Sub subsoundarcade()
	If tsoundarcade=0 Then tsoundarcade=1 Else tsoundarcade=0
	Sleep 200
End Sub
Sub soundocean
	'Exit Sub 
...
End Sub
Dim Shared As Double tsoundvoyage1,tsoundvoyage2,tsounddebby,tsoundvoyage,tsoundheresun,tsoundsomeone,tsounddebbylong
Dim Shared As Double tsoundvoyage3,tsoundnearness
...
End Sub
Sub soundheresun
	If Timer>tsoundheresun+189 Then '3m07
	  tsoundheresun=Timer
     'mcisendstring("set heresun time format ms",0,0,0)
     'mcisendstring("set heresun speed 972",0,0,0)
	  mcisendstring("play heresun from 0",0,0,0)
	EndIf   
End Sub
Sub soundsomeone
...
Function testivoyage() As Integer
	If Timer<tsoundvoyage+0.2 Or tradio=1 Then Return ivoyage
	tsoundvoyage=Timer
	If Timer>tsoundvoyage1+295-0.0001 And ivoyage=1 Then ivoyage=0
	If Timer>tsoundvoyage2+252-0.0001 And ivoyage=2 Then ivoyage=0
	If Timer>tsounddebby+81-0.0001 And ivoyage=3 Then ivoyage=0
	If Timer>tsoundheresun+189-0.0001 And ivoyage=4 Then ivoyage=0
	If Timer>tsoundsomeone+138-0.0001 And ivoyage=5 Then ivoyage=0
	If Timer>tsounddebbylong+272-0.0001 And ivoyage=6 Then ivoyage=0
	If Timer>tsoundnearness+191-0.0001 And ivoyage=7 Then ivoyage=0
Return ivoyage	
End Function
Sub soundvoyage(isong As Integer=0)
	If Timer<tsoundvoyage+0.2 Or tradio=1 Then Exit Sub
	tsoundvoyage=Timer
	If Timer>tsoundvoyage1+295-0.0001 And ivoyage=1 Then ivoyage=0
	If Timer>tsoundvoyage2+252-0.0001 And ivoyage=2 Then ivoyage=0
	If Timer>tsounddebby+81-0.0001 And ivoyage=3 Then ivoyage=0
	...
	If isong=11 And (ivoyage>=1 and ivoyage<=7) Then stopsoundvoyage():Sleep 200:ivoyage=0:Exit sub
	If ivoyage=0 Then
		stopsoundarcade()
	...
	If ivoyage=4 Then soundheresun
...
Sub stopsoundvoyage()
	...
	mcisendstring("stop heresun",0,0,0)	
...
Declare Sub inittownobject
    ...
    Dim Shared As uint pause,ianim,glnormals,restart,fullscreen=1,compas=1    
    Dim Shared As uint soltexture,arbretext(10),tronctext,buissontext(2),volanttext,volanttext2,arbreflowertext
    Dim Shared As uint cartext,md2horsetext(4),md2zebratext(3),grasstext(10),arbreflower2text
    Dim Shared As uint sunsettext,startext,radartext,pistetext,maptexture
    ...
    Dim Shared As uint seawavetext,watertext
...
Dim Shared As Single to1car,to2car,to1plane,to2plane
Dim Shared As Integer tshadow=1,tfoothorse=0
#Include "./movecar.bi"
...
If topview<=2 Then 
 ...
 ElseIf (mapautotext=webtext And planet=0) Or testsea=0 Then
	...
	Var txx=max(-1,min(2.0,0.5+k66*(x-xweb-256)*0.5/dxweb))
	gltexcoord2f(txx,tyy)
	'Var tyy=0.06+0.94*(y-yweb+dyweb)*0.48/dyweb
...
For j=1 To ishadowroc
     ...
     Var h=max(0.0,dr-rocdz(i))*1.3
     Var si2=1.0-h/(dr)
     Var auxyy=Sqr(1-si2*si2)*dr
     Var auxzz=h*min(2.0,dxyshadow/max(0.001,Abs(dzshadow)))
     'Var dr=auxyy*0.7
...
Function setbuildh(h As Single) As Single
	...
	If itown=5 Then hhh=h80+(h-h80)*1.1'*2
	If itown=4 Then hhh=h80+(h-h80)*0.8'*2 
	If itown=3 Then hhh=h80+(h-h80)*0.5'*2
	If itown=2 Then hhh=h80+(h-h80)*0.3'*2
	If itown=1 Then hhh=h80+(h-h80)*0.2'*2
	hhh=min(hh,hhh)
	Var kh=(h-h80)/(hh-h80)
...
Dim Shared As Integer itownp=1,tloadwebtext2=0
...
Dim Shared As String namenearroad(nnearroad)
Const As Integer nnear0road=200
...
Dim Shared As Single townwaynodevie(ntown,ntownnode),townwaynodeo1(ntown,ntownnode,nwaynode),townwaynodesize(ntown,ntownnode)
...
Dim Shared As Single townwaynodevie2(ntown,ntownnode),townwaynodeo12(ntown,ntownnode,nwaynode),townwaynodesize2(ntown,ntownnode)
...
Dim Shared As Integer townlock(ntown),lockterrain,lockterrainbmp,ttsetterrain(ntown,ntownnode),ttsetterrain2(ntown,ntownnode) 
...
Dim As Integer i 
For i=1 To 4000
	If lockterrain=0 Then lockterrain=1:Exit Sub
...
Dim As Integer i 
For i=1 To 4000
	If lockterrainbmp=0 Then lockterrainbmp=1:Exit Sub
	If quit=1 Then Exit Sub
	Sleep 2
Next
End Sub
...
Dim As Integer i 
For i=1 To 4000
	If townlock(ij)=0 Then townlock(ij)=1:Exit Sub
	If quit=1 Then Exit Sub 
	Sleep 2
Next
End Sub
...
Dim As integer ii,jj,ij,ij2,n,p
Dim As int64 id
...
If i2>-i50 And i2<i50 And j2>-i50 And j2<i50 Then
	   ...
	   If n>0 Then
	    For ii=1 To n'ntownnode
	      ...
	      If towniwaynode(ij2,ii)<2 Then Continue For 
	      Var ij=gettownij(townwaynodex(ij2,ii,1),townwaynodey(ij2,ii,1))
	      If ij=0 Then Continue For  	
		   iii=max2(1,min2(ntownnode,townnwaynode2(ij)+1))
		   For p=1 To iii-1
		   	If id=townwaynodeid(ij,p) Then
		   		'auxvar2+=1
		   		Continue For,For
		   	EndIf
		   Next
...
Dim Shared As Single kdistterrain=0.3
Dim Shared As Double tdistterrain
...
Dim Shared As Single scaleview=1,kdistterrain0=1,dfps=0,asktownlat=-99,asktownlon,kdistroad=1
Dim Shared As Single asktownlat2=-99,asktownlon2
Sub drawtowns
'iradar=0
'If (Timer>tdistterrain+max(0.3,min(1.0,0.15*kfps))) Then
If (Timer>tdistterrain+max(0.15,min(0.5,0.15*kfps))) Then
	tdistterrain=Timer
	kdistterrain=kdistterrain0
	Var kk=6.0/max(3.0,minfps) 
	If fpsmoy<6+dfps Then 
		kdistterrain=max(kk*0.050,kdistterrain*0.4)
	ElseIf fpsmoy<8+dfps Then 
		kdistterrain=max(kk*0.070,kdistterrain*0.6)
	ElseIf fpsmoy<11+dfps Then 
		kdistterrain=max(kk*0.10,kdistterrain*0.85)
	ElseIf fpsmoy<17+dfps Then 
		kdistterrain=max(kk*0.15,kdistterrain*0.9)
	ElseIf fpsmoy>21+dfps And (mz-mzsol0)>200 Then
		kdistterrain=min(kk*0.69,kdistterrain*1.05)'0.69
   ElseIf fpsmoy>21+dfps And (mz-mzsol0)<=200 Then 
			kdistterrain=min(kk*0.25,kdistterrain*1.05)'0.5
   ElseIf fpsmoy>30+dfps And (mz-mzsol0)<=200 Then 
			kdistterrain=min(kk*0.69,kdistterrain*1.05)'0.5
	EndIf
...
Dim Shared As Integer ishadowpanel,taddshadowpanel,taddshadowroc,trun
...
Dim Shared As Double train,timeaddshadowtree,timeishadow
...
Else
	stopsoundsauterelle
EndIf
If testivoyage()=0 And (trun>=0 Or plane>0) And tsoundarcade=1 And Timer>train Then
	If plane>0 Then
		If arcadesound=0 Then soundarcade
...
If ((Abs(nathaliex-nathaliex0)+Abs(nathaliey-nathaliey0)>250) Or namenathalie<>namenathalie0) _   
   And namenathalie<>"" And textspeak="" Then
	'textspeak="  i love you "+namenathalie
...
EndIf
taddshadowquad=0
If scaleview>0.9 And testposx0=1 Then 
 ...
 EndIf
EndIf
If scaleview>0.9 And time2>timeaddshadowtree+2 Then
...
Dim fogcolor(0 to 3) as single => {1,1,1, 1.0}
glFogi(GL_FOG_MODE, gl_linear)
glFogfv(GL_FOG_COLOR, @fogcolor(0))
glFogf(GL_FOG_DENSITY, 1.0)
glHint(GL_FOG_HINT, gl_nicest)'GL_DONT_CARE)
Var ifog1=ifog
If ifog=21 Then ifog1=max2(0,min2(20,Int(120000/(1000+wfog))))
...
Dim Shared As Single carx,cary,carz,cursorx,cursory,cursorz
Sub glplacecursor(ByVal x As Single,ByVal y As Single,ByVal z As Single=-40)
   glLoadIdentity()
...
End Sub
Sub drawradar2
If car>0 Then drawradar2car():Exit Sub 
...
End Select
End Function
Dim Shared As Single stationx,stationy,stationz,stationvie
Sub drawradar
Dim As Integer i
...
If plane=1 And volant>=1 And tourelle=0 Then
 drawradar2()
 glpushmatrix
 If car=0 Then	
...
'glenable GL_DEPTH_TEST
End Sub
Dim Shared As Single volantrots0
Sub drawvolant
 gldisable gl_depth_test	
 gldisable(gl_lighting)
 glenable gl_alpha_test
...
Declare Function getrandomnearroad(x As Single,y As Single,n0 As Integer,nco1 As Single,nsi1 As single)As Integer
...
For i=1 To nlady
   ...
   'ladyz(i)=getterrainheight(ladyx(i),ladyy(i))+12	
	If ladyz(i)<waterz-0.99 Then Continue For 
	If ladyz(i)<waterz-0.7 and ntownnear<1 Then Continue For
  	If tupdate=1 Then
  		ladyz(i)=getterrainheight(ladyx(i),ladyy(i))
...
Dim Shared As Single ncarvv(ncar)
...
Dim As integer i 	
	...
	Next
End Sub
Dim Shared As Integer ncari1,thighway,tinverse
Dim Shared As Double timeautopilot,timeautopilot2
Dim Shared As Single ncarv0,vautopilot0=5.5,ncarx0,ncary0,ncaro10,ncarcos100=1,ncarsin100,o1autopilot
Dim Shared As Single ncarz0,zroad,avgco1=1,avgsi1
Dim Shared As String myroadwayname
Dim Shared As Double tmyroadwayname
Sub drawncars()
Dim As Integer i,j
  ...
  If scaleview>0.9 Then 
	...
	If (Abs(ncarx(i)-mx-dmx0)>dmax Or Abs(ncary(i)-my-dmy0)>dmax) Then
	 If i=0 Then
		...
		ncarv(i)=4
	 ElseIf i>Int(1+ncar2*inearroad2/400) Then'/15
	 	ncarx(i)=-999999
	 	ncarz(i)=-999999
	 	ncariroad(i)=0
	 	Continue For 
	...
	Else
	   ...
	   x+=ncarvv(i)*kfps*ncarco1(i)*0.8
	   y+=ncarvv(i)*kfps*ncarsi1(i)*0.8		
	EndIf
	Var n=ncariroad(i)
	If time2>ncartimeroad(i)+4 Then n=0
	n=getrandomnearroad(x,y,n,ncarco1(i),ncarsi1(i))
	If n=0 And i=0 Then
		If asktownlat<-90 Then
			Var lat0=lat,lng0=lng
			mxytolatlng(mx+cos1*1000,my+sin1*1000)
   ...
   If n>0 Then
    ...
    tinverse=0
    If dxx<0 And onewaynearroad(n) And (i>0 Or inear0road>10) Then tinverse=1
    ...
    If tinverse Then 
  	   	If Rnd<0.5 Then nncaro1(i)=nncaro1(i)-140+(Rnd-0.5)*40
    EndIf
    If i=0 And tinverse=0 Then
    	If avgco1*co1+avgsi1*si1<0 Then
   ...
   If ncariroad(i)<>n Then
   	...
   	If Abs(ncarv(i))>1 then ncarv(i)*=0.7:nncarv(i)*=0.7
   EndIf
   ...
   If Abs(dcarx)>400 Or Abs(dcary)>400 Then kx=1
   ncarv(i)+=(nncarv(i)-ncarv(i))*kx
   ncarx(i)+=(dcarx)*kx
   ncary(i)+=(dcary)*kx
   Var do1=nncaro1(i)-ncaro1(i)
   While do1>180:do1-=360:Wend
   While do1<-180:do1+=360:Wend
   volantrots(i)=max(-45.0,min(45.0,do1+do1))
   ncaro1(i)+=do1*kx
   ncarz(i)=getterrainheight(ncarx(i),ncary(i))
  EndIf'scaleview      
...
If tautopilot=1 And time2>timeautopilot+3 And plane>0 And car>0 Then'And scaleview>0.9 Then
 If ncariroad(0)>0 And mytestbridge=0 Then timeautopilot2=time2-3.001
 If ncari1=0 And time2>timeautopilot2+3 Then
    ...
    If ncariroad(0)=0 Then
      ...
      volantrots(0)=-25*(2+ncarco1(0)*(ncarcos100+0.25*ncarsin100)+ncarsi1(0)*(ncarsin100-0.25*ncarcos100))
    EndIf
...
Var v0=v*vautopilot/max(1.0,vautopilot0)
...
Sub drawvolant2(x As Single=0.47,y As Single=0.768,z As Single=-30)
 ...
 glplacecursor(xmax*(0.5+kx*(x-0.5)),ymax*(0.5+k*(y-0.5)),z*k)
 If volantrot>120 Then volantrot=120
 If volantrot<-120 Then volantrot=-120
 glrotatef(volantrot+volantrots0,0,0,1)
 volantrot=0.75*volantrot+1e-10
...
Dim Shared As uint p51dlist,p51d0list,f14list,f140list,zero0list,c150list,c1500list
...
glplacecursor(xmax*0.5,ymax*0.689,-40)
gltexcarre4(75,51.33)'46) 
glpopmatrix
gldisable gl_alpha_test
glenable GL_DEPTH_TEST
End Sub
...
If typeavion=12 Then drawc1500
...
Dim Shared As Single kxsat,kysat,kzsat,heure2
Sub drawstars
gldisable GL_LIGHTING
gldisable GL_DEPTH_TEST
glEnable GL_TEXTURE_2D
glbindtexture(gl_texture_2d,startext)
If heure<6 Or heure>20 Or tsphere=1 Or planet=1 Then 
...
glpopmatrix
End Sub
Dim Shared As Single timeh,timedo1,timedo2
...
if tsun<0 then
  ...
  If heure>4 And heure<=6 Then
  	  aux=(heure-4)/2
  	  ksoleil=0.74*aux+(1-aux)*0.4
  EndIf     
  If heure>6 And heure<=8 Then
  	  aux=(heure-6)/2
  	  ksoleil=1.0*aux+(1-aux)*0.74
  EndIf
...
gltranslatef( mx-kxsoleil*5,my-kysoleil*5,mz+kzsoleil*5)
glrotatef(-heure*170/24,0,0,1)
glrotatef(-50*(1-Abs(heure-12)/12),0,1,0)
gldisable GL_LIGHTING
'glenable GL_LIGHTING
...
r0=Sqr(ksoleil):g0=ksoleil:b0=min(1.0,2.0*ksoleil)
...
For i=0 To ix
   For j=0 To iy
      ...
      kdo2=max(0.0,min(1.0,1-Abs(do2/70)))
   	r=max(0.0,min(1.0,r0*(0.6+kdo1*kdo2)))
		skydomer(i,j)=max(0.0,min(1.0,0.7+1.2*(r-0.7)))'*max(0.0,min(1.0,Abs(do1)/180))
		skydomeg(i,j)=max(0.0,min(1.0,0.7+1.2*(g0-0.7)))'*max(0.0,min(1.0,Abs(do1)/180))
		skydomeb(i,j)=max(0.0,min(1.0,0.7+1.2*(b0-0.7)))'*max(0.0,min(1.0,Abs(do1)/180))
   Next 		
Next 
...
Dim Shared As uint nhorse=17,md2ihorse(nhorse),testsoundshoot'nhorse=29
	...
	If Rnd<0.003*kfps Or rain<24 Then '10sec
	   ...
	   If Rnd<(1/6)*(krain*0.99) Then   '60sec
	      var ciel=Trim(guigettext("win.ciel"))
	   	...
	   	If test=1 Then
	   		train=Timer+(train-Timer)*(0.8+Rnd)/10'15
	   		soundthunder()
	   		rain=100
	   	EndIf
	   EndIf
...
If Timer<train+6 And scaleview>0.9 Then drawraindrop()
If Timer>train Then
	If rainsound=1 Then rainsound=0:stopsoundrain
	Exit Sub
EndIf
If rainsound=0 Then rainsound=1:soundrain 
...
If md2node->md2model<>0 Then
   ...
   If (dist)<(gldistmax*gldistmax) And (md2node->z)>waterz Then   	
...
Declare Function gettestnear0road(x As Single,y As Single)As Integer
Sub drawgrass
Dim As Integer i  	
...
distmax2=distmax+30000
'gldisable(gl_lighting)
glenable(gl_normalize)
glenable(gl_texture_2d)
Var nship22=nship2
If plane=0 Or car>0 Then nship22=6
For i=1 To nship2
  ...
  'If airshipy(i)>distmax Then airshipy(i)=distmax:airshipo1(i)+=90:setairship(i)=1
  While airshipx(i)<-distmax2:airshipx(i)+=2*distmax2:Wend
  While airshipx(i)>distmax2:airshipx(i)-=2*distmax2:Wend 
  While airshipy(i)<-distmax2:airshipy(i)+=2*distmax2:Wend 
  While airshipy(i)>distmax2:airshipy(i)-=2*distmax2:Wend 
  If pause=0 Then moveairship(i)
...
If plane>0 Or testztop=1 Then
 ...
 If dist20<70 And dist21>70 Then
 	mx=mx0-9*cos1*kfps:my=my0-9*sin1*kfps
 	mx+=kfps*9.5*sin1:my-=kfps*9.5*cos1:timecollide2=time2
 	If mytestroad2=0 Then o1-=1
 	tdist20=1
 ElseIf dist21<70 And dist20>70 Then
 	mx=mx0-9*cos1*kfps:my=my0-9*sin1*kfps
 	mx-=kfps*9.5*sin1:my+=kfps*9.5*cos1:timecollide22=time2
 	If mytestroad2=0 Then o1+=1
 	tdist20=1
 elseif (dist20<70 Or dist21<70)And testztop=1 And (tautopilot=0 Or plane=0) Then 
 	mx=mx0-cos1*(9+v)*kfps:my=my0-sin1*(9+v)*kfps
 EndIf 	
EndIf 
...
If time2>timebridge+4 And dist2<dist1+5 and dist3<dist1+5 And dist2>dist1-1 And dist1<70 Then
   if rnd<0.1 Then timecollide=time2
   If time2<timecollide+t4 Or v<-0.0001 Then
...
Dim As Integer i,j,k 	
    ...
    ishadowtree=0
    Var tflower=0
    If wtempmin>10 And wtempmin<22 Then tflower=1
    For i=1 To narbre2
     ...
     If changearbre>=1 Then
     	  ...
     	  If test>0 Then
     	  	  ...
     	  	  Else
     	  	  	  arbrez(i)=-99999
     	  	  EndIf
     	  ElseIf changearbre=2 And arbrez(i)<-999990 Then
     	  	  arbrez(i)=-999999
     	  ...
     	  Else'If testairport(arbrex(i),arbrey(i))=0 Then
     	  	  ...
     	  	  If arbrezi>1900 And Rnd<0.3 Then arbretype(i)=7'pine
     	  	  If arbrezi>2500 And Rnd<0.3 Then arbretype(i)=7
     	  	  If arbrezi>3200 And Rnd<0.3 Then arbretype(i)=7
     	  'Else
     	  '	  arbrez(i)=-999999'waterz-1
     	  EndIf
     EndIf
     If arbrez(i)>-99990 Then 
      ...
      EndIf 
     EndIf
  If arbrez(i)>min(1.0,waterz) And mz<(mzsol0+9000) And arbretype(i)<>9 Then 
   ...
   If x2>-400 And x2<1200 Then addshadowtree(i)	
   	/'If x2>(0.9*max(Abs(y2),Abs(z2))-arbrehouseh(i)-100) Then
         ...
         glcolor3f(1,1,1)
   		...
   		glpopmatrix
   	EndIf '/
   If x2>(Abs(y2)-150) Then
    If scaleview<0.9 Then
       If x2>1000 Then Continue For   
    Else	
    	ntree+=1
    EndIf
    If x2>(Abs(z2)-150) Then
     glenable gl_alpha_test
     If tflower=0 Or arbretype(i)<5 Or ((i Mod 3)=1) Then
    ...
    For i=1 To nbuisson
     ...
     If changebuisson>=1 Then
        'If testairport(buissonx(i),buissony(i),1000,110)=0 Then
     	  If gettestroad(buissonx(i)+dmx0,buissony(i)+dmy0)=0 Then
...
Sub displayback(tdraw As integer=1)
   ...
   If mapdisplay<4 Then  
    ...
    If ifog>0 And planet=0 Then glEnable(GL_FOG)
   EndIf 
    ...
    'freelockterrain()            	
End Sub
Dim shared as double tmxmove
dim shared as single mxmove,mymove,dkzoom=1.0,tkzoom=0,mzbridge=-999999
Sub display ()
...
If car>=1 And plane>0 And suspension>0.01 Then 'suspensions 
 ...
 o2+=(vm2-vm20)*5*suspension/(0.02+kfps/2)'*5*
...
If v>4 Then suspension=max(0.0,suspension-0.08*kfps)

    		If mapdisplay<>0 Then
    			If guitestkey(vk_left) Then mx-=4000:Sleep 200
    			If guitestkey(vk_right) Then mx+=4000:Sleep 200
    			If guitestkey(vk_up) Then my+=4000:Sleep 200
    			If guitestkey(vk_down) Then my-=4000:Sleep 200
    		EndIf 	
         ...
         If tautopilot=0 And v>10 And car>0 Then volume=min(volume,2200.0)
    		If plane>0 And car>0 And tautopilot=1 And mapdisplay<>1 And tourelle=0 Then
    		  ...
    		  If guitestkey(vk_up) Or guitestkey(vk_down) Or testjoy2 Then
    		  	  timeautopilot=time2-2.5
    		  	  avgo1=o1
    		  EndIf
    		  If v<1 Then timeautopilot=time2-2.5 
    ...
    If plane=0 Or car>0 Then
    	...
    	If (car>0 And plane>0) Or (plane=0 And (trun=1 Or guitestkey(vk_up))) Then
    		...
    		If Abs(mxmove-mx)>d180 Or Abs(mymove-my)>d180 Or (prop<1000*(1+sin2) And plane>0) Then
    			tmxmove=Timer
    			mxmove=mx:mymove=my 
    			If prop<100 And car>0 And tourelle=1 Then v=0:mx=mx0:my=my0
    		ElseIf Timer>tmxmove+4 Then
    			tmxmove=max(Timer-4,min(Timer,tmxmove+3))
    			mx+=d180*cos1:my+=d180*sin1
    			mxmove=mx:mymove=my
    		EndIf
    	Else
    		tmxmove=Timer
...
'getlockterrain()
   ...
   EndIf 
    ...
    glcolor3f(1,1,1)
    If md2nnode>0 And (mz<(mzsol0+9000)) And planet=0 And plane>0 And car=0 Then 
      For md2inode=1 To md2nnode
...
If planet=0 Then 
    ...
    If pause=0 And plane>0 And car=0 Then movehorses
...
If tsphere=0 And planet=0 Then
    ...
    If tourelle=0 And vie>0 Then testcollideforward()
...
EndIf 'planet
    ...
    EndIf
	 If (mz<(mzsol0+500) And (mzsol0-mzh)<waterz+1)Then'And tseashore=1 and webtext=0 Then
		If ntownnear<1 Then drawseashore()
	 EndIf
    'drawbullets()
    'freelockterrain()
    ...
    If plane=1 And tourelle=0 Then
      ...
      If volant=6 Then drawcockpit3  	
    	If volant>=1+1 And volant<=2 Then drawvolant
      gldisable gl_blend
      gldisable gl_alpha_test
    'ElseIf tshowradio=1 And tradio=0 Then
    '	tshowradio=0
    ...
    drawtest()
        ...
        Else 
           gldrawtext(Left(reverselocation,25),15,ymax-59,1.2)
       ...
       EndIf
        Var vv=min(v*3,Sqr((xkm-mx)*(xkm-mx)+(ykm-my)*(ykm-my)+(zkm-mz)*(zkm-mz))*fps/25)
        xkm=mx:ykm=my:zkm=mz
        If car>=-1 Then vkm+=(vv*4.75-vkm)*0.07*kfps Else vkm=v*17
        gldrawtext("prop= "+Str(Int(prop))+"  v= "+Str(Int(vkm))+"."+Str(Int(vkm*10+1000)Mod 10),15,ymax-35,1.2)
        gldrawtext("x= "+Str(Int((mx)*500/kscalex))+"+"+Str(Int(dmx0))+ _  
                 "  y= "+Str(Int((my)*500/kscalex))+"+"+Str(Int(dmy0))+ _ 
        ...
        If mapdisplay>0 Then
        	  glcolor3f(1,1,1)
        	  If dkzoom>1.1 Then
        	  	  gldrawtext("[Z]zoom = 1",10,320,1.2)
        	  Else 	  
        	  	  gldrawtext("[Z]zoom = 0",10,320,1.2)
        	  EndIf
        ...
        EndIf
        If tautopilot=1 And plane>0 And car>0 Then glcolor3f(1,1,1):gldrawtext("autopilot",10,290,1)
        If auxtest>0.5 Then 
          If tloadwebtext2>0 Then gldrawtext("loading "+Str(tloadwebtext2),10,310,1)
	 ...
	 Var n=ncariroad(0)
	 If plane=0 Or (car>0 And tautopilot=0) Then
	   n=getrandomnearroad(mx,my,n,cos1,sin1)
	   ncariroad(0)=n
	 EndIf  	
	 If n>0 And time2>tmyroadwayname+4 Then
...
Dim As Integer i  	
	...
	If i<>ncar2 Then
	  ncar2=i
	  For i=1 To ncar 
	   If i>Int(1+ncar2*inearroad2/400) Then'/15
	 	  ncarx(i)=-999999
	 	  ncarz(i)=-999999
	 	  ncariroad(i)=0
	   EndIf 
...
If plane=1 And car<>0 Then
	icar=myiplane:volant=4'5'8c150'7zero'icar=11
	If icar=7 Then volant=5
EndIf
...
If car<>0 Then
	...
	myicar=icar
Else
	icar+=1:If icar>9 Then icar=7
	volant=6
	If icar=9 Then volant=4
	myiplane=icar
EndIf
selectcomboindex("win.combo1",icar)
Sleep 100
	...
	For jjx=-i50 To i50 Step i50'Step dij
		...
		Else 
			dii=3
						...
						EndIf
						h=-999999
						If testinit=1 Then test=0
						If test=0 And ntown22<ntown And testinit>=0 Then
							If tmaptown(ntown22)<0 Then tmaptown(ntown22)=0
							townx(ntown22)=jjx*dij+jjx0
							towny(ntown22)=iix*dij+iix0
							If testinit=1 Then townz(ntown22)=-999999'(h-74+dh0-dhmareemax2)*scalez'+68.5
						   towntype(ntown22)=1
						   Select Case towntype(ntown22)
						   	Case 1
						   		dii=3
						   		For ii=i-dii To i+dii
						   		 For jj=j-dii To j+dii
						   			If testinit=1 Then
						   			   townbuild(ntown22,ii-i+dii,jj-j+dii)=0
						   			   townh(ntown22,ii-i+dii,jj-j+dii)=0
						   			Else    	
						   			   If townbuild(ntown22,ii-i+dii,jj-j+dii)>0 Then tmaptown(ntown22)=1
						   			EndIf 
						   			'h1=h'map(ii,jj)
						   			If testinit=1 Then towndz(ntown22,ii-i+dii,jj-j+dii)=-999999'(h1-74+dh0-2-dhmareemax2)*scalez
						   		   'If towndz(ntown2,ii-i+dii,jj-j+dii)<waterz Then
						   		   '	townbuild(ntown2,ii-i+dii,jj-j+dii)=0
						   		   'EndIf
						   		 Next
						   	...
						   	Case Else
						   		...
						   		For ii=i-dii To i+dii
						   		 For jj=j-dii To j+dii
						   			If townbuild(ntown22,ii-i+dii,jj-j+dii)>0 Then tmaptown(ntown22)=1
						   			h1=h'map(ii,jj)
						   			towndz(ntown22,ii-i+dii,jj-j+dii)=(h1-74+dh0-2-dhmareemax2)*scalez
						   		   'If towndz(ntown22,ii-i+dii,jj-j+dii)<waterz Then
						   		   '	townbuild(ntown22,ii-i+dii,jj-j+dii)=0
						   		   'EndIf
						   		 Next
						   		Next
  ...
  If tmaptown(i)>=0 Then 	
	For ii=0 To 6
		For jj=0 To 6
	      ...
	      EndIf    
	      If testinit=1 Then
	      	townddx(i,ii,jj)=0'towndx(i,ii,jj)
	         townddy(i,ii,jj)=0'towndy(i,ii,jj)
	      EndIf
		Next
	Next
	townx(i)=k8*512*scalex*(townx(i)/x960-0.5)*x960/1200
	towny(i)=-k8*512*scalex*(towny(i)/x960-0.5)*x960/1200-260
	If di<>0 Or dj<>0 Or tmapchanged=1 Then 
	 If townz(i)<-999998 Or tmapchanged=1 Then
		h=getterrainheightfast(townx(i)-dmx0,towny(i)-dmy0)
		If h>-999998 Then 
		 If h<waterz Then
		 	tmaptown(i)=-1
		 Else 
		   ...
		   For ii=0 To 6
			  For jj=0 To 6
			 	towndz(i,ii,jj)=getterrainheightfast(towndx(i,ii,jj)-dmx0,towndy(i,ii,jj)-dmy0)
			  Next
		   Next
		 EndIf  
		EndIf
	...
	For jjx=-i50 To i50 Step i50'Step dij
		...
		Else 	
   		dii=3
						...
						h=-999999
						If testinit=1 Then test=0
						If test=0 And ntowp22<ntowp And testinit>=0 Then
							If tmaptowp(ntowp22)<0 Then tmaptowp(ntowp22)=0
							towpx(ntowp22)=jjx*dij+jjx0
							towpy(ntowp22)=iix*dij+iix0
							If testinit=1 Then towpz(ntowp22)=-999999'(h-74+dh0-dhmareemax2)*scalez'+68.5
						   towptype(ntowp22)=1
						   Select Case towptype(ntowp22)
						   	Case 1
						   		dii=3
						   		For ii=i-dii To i+dii
						   		 For jj=j-dii To j+dii
						   			If testinit=1 Then
						   			   towpbuild(ntowp22,ii-i+dii,jj-j+dii)=0
						   			   towph(ntowp22,ii-i+dii,jj-j+dii)=0
						   			Else    	
						   				If towpbuild(ntowp22,ii-i+dii,jj-j+dii)>0 Then tmaptowp(ntowp22)=1
						   			EndIf
						   			'h1=h'map(ii,jj)
						   			If testinit=1 Then towpdz(ntowp22,ii-i+dii,jj-j+dii)=-999999'(h1-74+dh0-2-dhmareemax2)*scalez
						   		   'If towpdz(ntowp22,ii-i+dii,jj-j+dii)<waterz Then
						   		   '	towpbuild(ntowp22,ii-i+dii,jj-j+dii)=0
						   		   'EndIf
						   		 Next
						   		Next
						   	Case Else
						   		...
						   		For ii=i-dii To i+dii
						   		 For jj=j-dii To j+dii
						   			If towpbuild(ntowp22,ii-i+dii,jj-j+dii)>0 Then tmaptowp(ntowp22)=1
						   			h1=h'map(ii,jj)
						   			towpdz(ntowp22,ii-i+dii,jj-j+dii)=(h1-74+dh0-2-dhmareemax2)*scalez
						   		   'If towpdz(ntowp22,ii-i+dii,jj-j+dii)<waterz Then
						   		   '	towpbuild(ntowp22,ii-i+dii,jj-j+dii)=0
						   		   'EndIf
						   		 Next
...
ntown2=ntowp2
If testinit<=1 Then
 'If tinittowp=1 Then loadtowpvie()
 /'For i=1 To ntowp2
			'i=(x960*(0.5+(dmx0/scalex)/(k8*512)))
			'j=(x960*(0.5-(dmy0/scalex)/(k8*512)))
	'dmx0=scalex*(k8*512)*mapx0/x960
	'dmy0=scalex*(k8*512)*mapy0/x960
  If tmaptowp(i)>=0 Then 	
	For ii=0 To 6
		For jj=0 To 6
			towpdx(i,ii,jj)=k8*512*scalex*((towpx(i)+(-3+jj)*100/kscalex)/x960-0.5)*x960/1200
	      towpdy(i,ii,jj)=-k8*512*scalex*((towpy(i)+(-3+ii)*100/kscalex)/x960-0.5)*x960/1200-260
	      If (Rnd<0.3 Or towpvie(i,ii,jj)>0)And tinittowp=1 Then 
	        towpvie0=200+towph(i,ii,jj)*0.5
	        towpvie(i,ii,jj)=min(towpvie0,max(-90.0,towpvie(i,ii,jj)+towpvie0/5))
	      EndIf    
	      If testinit=1 Then
	      	towpddx(i,ii,jj)=0'towpdx(i,ii,jj)
	         towpddy(i,ii,jj)=0'towpdy(i,ii,jj)
	      EndIf
		Next
	Next
	towpx(i)=k8*512*scalex*(towpx(i)/x960-0.5)*x960/1200
	towpy(i)=-k8*512*scalex*(towpy(i)/x960-0.5)*x960/1200-260
	If di<>0 Or dj<>0 Or tmapchanged=1 Then 
	 If towpz(i)<-999998 Or tmapchanged=1 Then
		h=getterrainheightfast(towpx(i)-dmx0,towpy(i)-dmy0)
		If h>-999998 Then
		 If h<waterz Then
		 	tmaptowp(i)=-1
		 Else 
		   ...
		   For ii=0 To 6
			  For jj=0 To 6
			 	towpdz(i,ii,jj)=getterrainheightfast(towpdx(i,ii,jj)-dmx0,towpdy(i,ii,jj)-dmy0)
			  Next
		   Next
		 EndIf   
		EndIf
...
For x=0 To 512-1
	For y=0 To 512-1
		pix=getpixel(bmpdc,Int(0.1+bmpx*x/512),Int(0.1+bmpy*y/512))
...
If time2>timeseashore+2 Or tupdateseashore=1 Then
	...
	seashorex=Int(mx/d500)*d500
	seashorey=Int(my/d500)*d500
	dx=400:dz=waterz-0.11
	dx2=dx*2
	seashoreo1=1
	If getterrainheight(mx-dx,my)<dz And  getterrainheight(mx-dx2,my)<dz Then seashoreo1=180
	If getterrainheight(mx+dx,my)<dz And  getterrainheight(mx+dx2,my)<dz Then seashoreo1=0
	If getterrainheight(mx,my-dx)<dz And  getterrainheight(mx,my-dx2)<dz Then seashoreo1=90
	If getterrainheight(mx,my+dx)<dz And  getterrainheight(mx,my+dx2)<dz Then seashoreo1=-90
EndIf
...
Return h*scalez
End Function
Function setterrainheight(ByVal x As Single,ByVal y As Single,xco1 As Single,xsi1 As single) As Single
Dim As Single h,dx,dy,z00,z10,z01,z11 
Dim As Integer i,j,i1,j1
If orbit=0 And planet=1 Then Return -99999
	i=Int(x/scalex+100000)-100000
	...
	'While i1>=612:i1-=512+200:Wend
	'While j1>=612:j1-=512+200:Wend 
If tsetterrain(i1,j)<=4 Or tsetterrain(i,j1)<=4 Then
	z00=terrain0(i,j)
   z10=terrain0(i1,j)
   z01=terrain0(i,j1)
   z11=terrain0(i1,j1)
...
Sub initarbres
   ...
   If arbretext(1)=0 Then 
    For i=1 To 7
   	arbretext(i)=guiloadtexture("media/arbre"+Str(i)+".jpg",130)'180)
    Next
...
Dim Shared As uint disthorse=12000
...
For i=1 To nhorse
  ...
  Else 
    ...
    Else 
    	...
    	Else
    	  If i<=23 Then 	
          If max(Abs(horsex(18)-md2node->x),Abs(horsey(18)-md2node->y-(i-17)*150))<500 Then
   	     vhorse=4.5
          Else 
  	        vhorse=5.7
          EndIf
    	  Else
    	  	 If i<=27 Then 
            If max(Abs(horsex(24)-md2node->x),Abs(horsey(24)-md2node->y-(i-23)*50))<500 Then
   	       vhorse=4.5
            Else 
  	          vhorse=5.7
            EndIf
    	  	 Else 
            If max(Abs(horsex(28)-md2node->x),Abs(horsey(28)-md2node->y-(i-28)*50))<400 Then
   	       vhorse=4.5
            Else 
  	          vhorse=5.7
            EndIf
  ...
  EndIf
  If Rnd<0.02*kfps Then
  	  If i=1 Or i=7 Or i=13 Or i=18 Or i=24 Or i=28 Then 
  	    If Rnd<0.05 Then
  	    	horseo1(i)+=(Rnd-0.5)*120
  	    	If (Abs(horsex(i))+Abs(horsey(i)))>(distmax-7000) Then
  	    		If Abs(horsex(i))>Abs(horsey(i)) Then
  	    			If horsex(i)>0 Then horseo1(i)=180 Else horseo1(i)=0
  	    		Else
  	    		   If horsey(i)>0 Then horseo1(i)=-90 Else horseo1(i)=90
  	    	   EndIf 
  	    	EndIf
  	    EndIf
  	  Else 
  	    ...
  	    Else 
  	    	...
  	    	Else
  	    		...
  	    		Else 
  	    		  If i<=23 Then 
  	    		  	 dmx=(horsex(18)-horsex(i))*si1-(horsey(18)-horsey(i)-(i-17)*150)*co1
  	    		  Else
  	    		  	 If i<=27 Then 
  	    		  	 	dmx=(horsex(24)-horsex(i))*si1-(horsey(24)-horsey(i)-(i-23)*50)*co1
  	    		  	 Else 
  	    		  	 	dmx=(horsex(28)-horsex(i))*si1-(horsey(28)-horsey(i)-(i-27)*50)*co1
  	    		  	 EndIf
  	    		  EndIf
  	    		EndIf 		
  	    	EndIf
...
If plane=0 Then 
   ...
   EndIf  
    ...
    If guitestkey(vk_up)Or guitestkey(vk_space) Then mx+=vv*cos1*kfps:my+=vv*sin1*kfps
...
Else
    ...
    If guitestkey(vk_r)<>0 Then
    	Sleep 200
	   If (piste=1 Or tstation=2 Or trepair=1) And Abs(v)<0.1 And boatwar(myboat)=0 Then
...
Function waitbusy() As Integer
      ...
      EndIf
		...
		If tloadwebtext2<>0 Or tinittown<>0 Then
			guinotice("i am busy, try later "+Str(tloadwebtext2)+"/"+Str(tinittown))
			Return 0 
		EndIf
	   Return 1
End Function
...
msg+="V  => visor     C => compas    G => change car/plane"+cr
...
Sub subautopilot
	...
	Else	
		...
		If resp="yes" Then tautopilot=0
	EndIf
	soundavion
End Sub
Dim Shared As Integer mousex,mousey,mousex0,mousey0,tmouse
Sub subrightmouse
SetCursorPos(Int(xmax/2),Int(ymax/1.6))
...
If tradio=0 Then 
   ...
   stopsoundvoyage()
  	...
  	EndIf
  	printgui("win.radio","radio on")
Else
	tradio=0
	printgui("win.radio","radio off")
EndIf
guisetfocus("win.graph")	
End Sub '/

Dim Shared As Integer nxweb=4,imxweb=1
Dim Shared As Single mxweb(nxweb),myweb(nxweb)
Sub resetmxweb()
Dim As Integer i  	
For i=1 To nxweb
	mxweb(i)=999:myweb(i)=999
Next
imxweb=1
End Sub
#Include "textures.bi"
...
mz11=-999999
        ...
        eurofighterlist=0:eurofighter0list=0:c150list=0:c1500list=0:ballonlist=0
        fighterlist=0:fighter0list=0:spaceshiplist=0:spaceship0list=0
        ...
        Else
           ...
           winx=0:winy=0
        EndIf    
        button("win.test","quit",@subquit,10,ymax+20,45,30) 
        ...
        Var tquit=Timer,tframe=tquit-5 
        quit2=0
        While (quit=0 And guitestkey(vk_escape)=0)Or tframe<tquit
         	...
         	testmaree()
         	If (itime Mod 20)=1 Then settown()
        	   ...
        	   If guitestkey(vk_t)<>0 And guitestkey(vk_control)<>0 Then
        	   	If guitestkey(vk_shift)=0 Then subnarbre Else subsavetown
        	   EndIf
...
If (planet=0 And orbit=1)And topview=0 And mapdisplay<>4 And mapdisplay<>6 And (tinternet>=1 Or quit=1) Then
	If quit=1 Then
		quit2=1:tquitweb=1
  ...
  If tframe<tquit Or quit=0 Then 
   ...
   EndIf
   If (mz-mzsol0)<-100 and v<5 Then
   	dxx=1800:dyy=1800
   	If zoom<>16 And webtext>0 And Timer>tloadwebtext+t10 Then
   		test=1
   		zoom=16
   	EndIf
   ElseIf (mz-mzsol0)<700 Then'<500 And (plane=0 Or car>0)Then
   	'dxx=7500:dyy=7500
   	dxx=3500:dyy=3500
   ...
   If mapdisplay=1 And guitestkey(vk_z) Then
   	If dkzoom<1.1 Then dkzoom=6 Else dkzoom=1
   	kzoom=4/dkzoom
   	tkzoom=1
   	tquitweb=1
   	Sleep 300 
   EndIf
    ...
    If xwebzoom1<mxx-dxx1*kzoom*1.5 Or _
       xwebzoom1>mxx+dxx1*kzoom*1.5 Or _
       ywebzoom1<myy-dyy1*kzoom*1.5 Or _
       ywebzoom1>myy+dyy1*kzoom*1.5 Or tkzoom>0.1 Then
   ...
   Var mxx=mx+dmx0+cos1*dxx*0.7
   Var myy=my+dmy0+sin1*dyy*0.7
   ...
   If tquitweb=1 Or quit=1 Then
   	...
   	EndIf
   EndIf
   If test=0 And tloadwebtext2=0 And skyauto<=1 Then
   	testgetweather()
   	If tsubciel=1 Then tsubciel=0:subciel():setfog()
   EndIf
   If test=1 And tloadwebtext2<>2 And ((mz-mzsol0)<100 Or Abs(sin3)<0.60) Then '<0.67
   	tloadmapdisplay=0
      ...
      If (Abs(worldx-worldx00)+Abs(worldy-worldy00))>0.001 Then 
         ...
         Sleep 100
      EndIf
      If tloadwebtext2<>2 Then 
         tloadwebtext2=2
   	   threaddetach(ThreadCreate(@loadwebtext))
   	EndIf    
   ElseIf test=11 And tloadwebtext2<>2 Then
   	   tloadwebtext2=2
   	   If tkzoom>0.1 Then 
   	    tkzoom=0
   	    tquitweb=0
   	   ...
   	   xwebzoom1=mx+dmx0:ywebzoom1=my+dmy0
   	   xweb11=xweb
   ...
   If tloadwebtext2=1 Then
     ...
     Else 
      tloadwebtext2=2
      tloadwebtext=0
      xweb=xweb1:yweb=yweb1:dxweb=dxweb1:dyweb=dyweb1
      subsettupdate()
      If testworld=0 Then
      	y+=(yweb-y)*0.5
      EndIf
   ...
   ElseIf tloadwebtext2=33 Or tinittown=3 Then
      ...
      'updateworldobjects
      'initterrainlines
   ElseIf tloadwebtext2=0 And tinittown=0 And tinittown0<=0 And topentown=1 And Timer>tloadwebtext+t10+0.3 And _  
   	    abs(sin3)<0.60 And quit2=0 And tinternet>=2 And _   
      ...
      Var kdx=1.0
      If (mz-mzsol0)<690.500 Then kdx=0.7
      ...
      If (mz-mzsol0)<690.500 Then kz-=0.65
      ...
      dlatmax*=min(1,kz)
   	If (mz-mzsol0)<700 Then
   	 If imxweb>2 Then imxweb=1	
   	 For i=3 To nxweb
   	 	mxweb(i)=999:myweb(i)=999
   	 Next
   	EndIf   
   	If asktownlat<-90 And asktownlat2<-90 Then 
   	 For i=1 To nxweb
   	 ...
   	 EndIf
   	EndIf  
   	If test2=1 Or asktownlat>-90 Or asktownlat2>-90 Then
   		t11=0
   		tloadwebtext2=2
   		If asktownlat<-90 And asktownlat2<-90 Then 
   		 If (mz-mzsol0)>700 Then              
   		 ...
   		 Else 
   			imxweb+=1:If imxweb>2 Then imxweb=1
      		mxweb(imxweb)=lngtown1
      		myweb(imxweb)=lattown1
      	 EndIf 
      	EndIf
  	   	...
  	   	lat0=lat:lng0=lng
  	   	mxytolatlng(mx+dmx0+100000,my+dmy0+100000)
  	   	kmxlat=100000/max(0.0000001,Abs(lat-latmx0))
  	   	lat=lat0:lng=lng0
  	   	tquitweb=0
  	   	o1town=o1
  	   	If asktownlat>-90 Then
...
Close #file''If townchanged=1 Or Rnd<0.2 Then savetownvie()
