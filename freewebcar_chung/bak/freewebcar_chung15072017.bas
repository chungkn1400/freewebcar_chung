...
End Sub
Dim Shared As Integer trikku=-1
Randomize
...
Sub setsoundarcade()
	...
	If trikku<0 Then
      ...
      If Rnd<0.5 Then trikku=2:soundfic="sounds/luck.mp3"
	Else 
...
Sub initsounds
   ...
   soundfic="sounds/magic_is_you.mp3"
...
Sub setsoundvol
	...
	mcisendstring("setaudio heresun volume to "+Str(Int(1.82*vol)),0,0,0)
	...
	mcisendstring("setaudio magic volume to "+Str(Int(3.0*vol)),0,0,0)
...
End Sub
Dim Shared As Double timesoundarcade
Sub soundarcade	
...
Dim Shared As Double tsoundvoyage3,tsoundnearness,tsoundjunk,tsoundgrantorino,tsoundmagic
...
End Sub
Sub soundmagic
	If Timer>tsoundmagic+226 Then '3m44
	  tsoundmagic=Timer
...
Function testivoyage() As Integer
	...
	tsoundvoyage=Timer
	If Timer>tsoundvoyage1+223-0.0001 And ivoyage=1 Then ivoyage=0
	...
	If Timer>tsoundjunk+156/0.95-0.0001 And ivoyage=8 Then ivoyage=0
	If Timer>tsoundgrantorino+302-0.0001 And ivoyage=9 Then ivoyage=0
	If Timer>tsoundmagic+226-0.0001 And ivoyage=10 Then ivoyage=0
Return ivoyage	
End Function
Sub soundvoyage(isong As Integer=0,text As String="")
	...
	If tTimer>tsoundheresun+189-0.0001 And ivoyage=4 Then ivoyage=0
	...
	If tTimer>tsoundmagic+226-0.0001 And ivoyage=10 Then ivoyage=0
	...
	If isong=11 And (ivoyage>=1 and ivoyage<=10) Then stopsoundvoyage():Sleep 200:ivoyage=0:Exit sub
	If ivoyage=0 Then
		stopsoundarcade()
...
Dim Shared As Single to1car,to2car,to1plane,to2plane,retroto1,retroto2
Dim Shared As Integer tshadow=1,tfoothorse=0,typeautopilot
#Include "./movecar.bi"
...
For i=-100 To 612
	...
	Next
Next
If Abs(mxold)>0.001 And mzold>-scalez And mzold<1900*scalez And Abs(scalezold-scalez)<0.001 Then
	Var dz=(mzold-getterrainheight(mxold,myold))/scalez
	For i=-100 To 612
		For j=-100 To 612
			terrain22(i,j)+=dz
			terrain(i,j)+=dz
...
Dim Shared As Integer itownp=1,tloadwebtext2=0,testweb,testweb2
...
Dim Shared As Integer mywayroadij,mywayroadi  
Sub getlockterrain()
'Exit sub
Dim As Integer i 
For i=1 To 4000
...
Dim Shared As Integer copydoubles,nbuilds
...
Dim Shared As Double train,timeaddshadowtree,timeishadow,train2
...
Else
	stopsoundsauterelle
EndIf
If testivoyage()=0 And (trun>=0 Or plane>0) And (tsoundarcade=1 Or plane=0) And Timer>train2 Then
	If plane>0 And (time2<timesoundarcade+2099 Or tautopilot=0) Then
		If arcadesound=0 Then soundarcade
...
taglcompile+=1:If taglcompile>30 Then taglcompile=1'0
If testtownshow=1 Then
...
Sub displaymap
 ...
 ElseIf mapdisplay=1 then 
   ...
   Else
   	...
   	If dkzoom<0.6 Then
   		kx*=0.25
   	ElseIf dkzoom>5 Then
   		kx*=4
   	elseIf dkzoom>10 Then
   		kx*=8
   	EndIf 	
...
Dim Shared As String myroadwayname,myroadwayid
  ...
  If scaleview>0.9 Then 
   ...
   Var kx=min(0.7,0.03*kfps)
	If i>0 And Rnd<0.5 Then 
	  ncarx(i)+=kfps*ncarco1(i)
     ncary(i)+=kfps*ncarsi1(i)		
	  nncarx(i)+=kfps*ncarco1(i)
     nncary(i)+=kfps*ncarsi1(i)		
...
If tautopilot=1 And time2>timeautopilot+3 And plane>0 And car>0 Then'And scaleview>0.9 Then
 ...
 If ncari1=0 And time2>timeautopilot2+3 Then
    ...
    If ncariroad(0)=0 Then
    	Var k25=25.0
    	If (Int(Timer/40)Mod 2)=0 Then k25=-30
...
End if
     ...
     glpopmatrix '/
If 1 Then
 If tdark=1 And plane>0 And car>0 Then
 	Var dr=20.0
   gllightfv(gl_light0,GL_position,glparam4f(mx+dr*cos1*cos2,my+dr*sin1*cos2,mz+dr*sin2,1)) 'w=1 position GL_POSITION
   glenable gl_lighting
   glenable gl_normalize
 Else 
   gllightfv(gl_light0,GL_position,glparam4f(mx-kxsoleil*9,my-kysoleil*9,mz+kzsoleil*9,0))'w=0,directional
...
dim shared as single avgv,avgmx,avgmy,mxmove,mymove,o1move,tkzoom=0,mzbridge=-999999
Sub display ()
...
If v>4 Then suspension=max(0.1,suspension-0.08*kfps)
    ...
    If plane=0 Or car>0 Then
    	...
    	If (car>0 And plane>0) Or (plane=0 And (trun=1 Or guitestkey(vk_up))) Then
    		...
    		EndIf
    		Var d180=max(50.0,5.0*avgv)'180.0
    		If plane=0 Then d180=80
    		Var k01=0.03*kfps
    		avgv+=(vkm-avgv)*k01:avgmx+=(mx-avgmx)*k01:avgmy+=(my-avgmy)*k01
    		If Abs(mxmove-avgmx)>d180 Or Abs(mymove-avgmy)>d180 Or Abs(o1move-o1)>60 Or (prop<500*(1+sin2) And plane>0) Or _  
    		   mapdisplay>0 Then
    			tmxmove=Timer
   ...
   EndIf  
    ...
    drawtest()
       ...
       EndIf
        ...
        If mapdisplay>0 And auxtest<0.699 Then
        	  glcolor3f(1,1,1)
        	  If dkzoom>6.1 Then
        	  	  gldrawtext("[Z]zoom = 2",10,320,1.2)
        ...
        EndIf
        If tautopilot=1 And plane>0 And car>0 Then
    ...
    EndIf
	 ...
	 EndIf 	
        ...
        gldisable gl_depth_test
        If time2<tspeak+8 And (plane=0 Or car>0 Or myroadwayname="") Then
...
If dx<=(1.0-dy) Then 
  ...
  If tsetterrain(i,j)<=t4 Then tsetterrain(i,j)+=1:terrain(i,j)=z00+(h-z00)*(Abs(dx*xsi1-dy*xco1))*d100/max(d10,Abs(dx)+Abs(dy))
  If tsetterrain(i1,j)<=t4 Then tsetterrain(i1,j)+=1:terrain(i1,j)=z10+(h-z10)*(Abs((1-dx)*xsi1-dy*xco1))*d100/max(d10,Abs(1-dx)+Abs(dy))
  If tsetterrain(i,j1)<=t4 Then tsetterrain(i,j1)+=1:terrain(i,j1)=z01+(h-z01)*(Abs(dx*xsi1-(1-dy)*xco1))*d100/max(d10,Abs(dx)+Abs(1-dy))
Else 
     h=( (1-dx)*(z01-z11) +(1-dy)*(z10-z11) +z11 )
  'If tsetterrain(i1,j)<=t4 Then tsetterrain(i1,j)+=1:terrain(i1,j)=z10+(h-z10)*(Abs((1-dx)*xsi1-dy*xco1)/max(0.01,Abs(1-dx)+Abs(dy)))'*d100/max(d10,Abs(1-dx)+Abs(dy))
  'If tsetterrain(i,j1)<=t4 Then tsetterrain(i,j1)+=1:terrain(i,j1)=z01+(h-z01)*(Abs(dx*xsi1-(1-dy)*xco1)/max(0.01,Abs(dx)+Abs(1-dy)))'*d100/max(d10,Abs(dx)+Abs(1-dy))
  ...
  If tsetterrain(i1,j)<=t4 Then tsetterrain(i1,j)+=1:terrain(i1,j)=z10+(h-z10)*(Abs((1-dx)*xsi1-dy*xco1))*d100/max(d10,Abs(1-dx)+Abs(dy))
  If tsetterrain(i,j1)<=t4 Then tsetterrain(i,j1)+=1:terrain(i,j1)=z01+(h-z01)*(Abs(dx*xsi1-(1-dy)*xco1))*d100/max(d10,Abs(dx)+Abs(1-dy))
  If tsetterrain(i1,j1)<=t4 Then tsetterrain(i1,j1)+=1:terrain(i1,j1)=z11+(h-z11)*(Abs((1-dx)*xsi1-(1-dy)*xco1))*d100/max(d10,Abs(1-dx)+Abs(1-dy))
EndIf 
Return h*scalez
End Function
...
msg+="ctrl+T => number of trees   /   ctrl+shift+T => set retro view"+cr
...
mz11=-999999
        ...
        button("win.googlemap","googlemap",@subgooglemap,882,ymax+24,85,24) 
...
If Timer>tupdate+0.2 And (planet=0 And orbit=1)And topview=0 And mapdisplay<>4 And mapdisplay<>6 And (tinternet>=1 Or quit=1) Then
  ...
  If tframe<tquit Or quit=0 Then 
   ...
   If (tloadwebtext2=2) And Timer>t222+30 and httpon<>1 Then
   	t222=Timer:tloadwebtext2=0
   EndIf 	
   ...
   EndIf
   If guitestkey(vk_g) And guitestkey(vk_shift) Then subgoogle()
      ...
      dlatmax*=min(1,kz)
   	...
   	If test2=1 Or asktownlat>-90 Then'Or asktownlat2>-90 Then
   		...
   		If asktownlat<-90 Then'And asktownlat2<-90 Then 
   		 ...
   		 Else 
   			imxweb+=1:If imxweb>3 Then imxweb=1
      		mxweb(imxweb)=lngtown1
      		myweb(imxweb)=lattown1
      		dmxweb(imxweb)=dlatmax*kdx*0.5
   		 EndIf 
