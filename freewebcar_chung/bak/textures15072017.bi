...
Dim Shared As uint cocacolatext,marisoltext
Sub resettextures()
...
If googleerror=0 Then 
	webidata=httppost(hostname,path+apikey)
Else
	webidata=0
EndIf 
If webidata=0 Then 
...
If googleerror=0 Or tinittown0>1 Then 
	webidata=httppost(hostname,path+apikeystat)
Else
	webidata=0
EndIf 
If webidata=0 Then 
...
Var hostname="maps.googleapis.com"
If googleerror=0 Then 
	webidata=httppost(hostname,path2+apikey)
Else
	webidata=0
EndIf 
If webidata=0 Then 
	 ...
	 If j<=0 Then 
     	...
     	If j>0 Then
  	    waylat(i)=mylat(j)
 	  	 waylon(i)=mylon(j)
     	Else
     	 If InStr(wtext1,"""highway""")>1 Then
  	  	     	If myiasknode<mynasknode Then
  	  	     		'myiasknode+=1
  	  	     		'myasknodeid(myiasknode)=id
  	  	     		testaskhighway=1
	...
	If Abs(dx)<(r+dr) Then
		...
		If Abs(dy)<(r+dr) Then
			...
			If Abs(dx*si1-dy*co1)<(r+0.30) Then
                ...
                If dxxx<Abs(dcos1)*(20+r0) Then
               	dxxx=Abs(dcos1)*(20+r0)
               	idxxx=i
                EndIf
 ...
 For j=2 To n
   ...
   For jj=1 To di
     ...
     Else  
      ...
      z1=z0
     EndIf
     If dist>4000 Then Continue For 
      'z0=min(z0,zmax):z1=min(z1,zmax)
      Var dz=0.0
      Var txx=tx
      If trail=0 Then 
...
If dtweb<18 And fpsmoy>14 Then 
...
Var url="https://"+hostname+path+mapstyle3+apikey
ShellExecute(NULL,"open",url,NULL,NULL,SW_SHOWmaximized)
'ShellExecute(NULL,"open","iexplore.exe",url,NULL,SW_SHOWmaximized)
sleep 1000	
